  ...
  return (
    <div className="preview-chart" aria-label="Trend chart">
      ...
      <div className="chart-plot-with-scale"><div className="chart-y-axis">{[1, .8, .6, .4, .2, 0].map((step) => <span key={step}>{formatValue(scaleMax * step)}</span>)}</div><div className="preview-chart-art">
        ...
        {activePoint !== null && <ChartTooltipPosition index={activePoint} count={pointCount}><strong>{samples[activePoint].tooltipLabel}</strong>{pointValues(activePoint).map((item) => <div key={item.label}><i style={{ background: item.color }} /><span>{item.label}</span><b>{item.value.toLocaleString()}{scaleMax === 100 ? "%" : ""}</b></div>)}</ChartTooltipPosition>}
      </div></div>
...
function ChartTooltipPosition({ index, count, children }: { index: number; count: number; children: React.ReactNode }) {
  const progress = index / Math.max(1, count - 1);
  const style: CSSProperties = progress > .5
    ? { right: 8, left: "auto" }
    : { left: `${Math.max(8, progress * 72)}%`, right: "auto" };
  return <div className="chart-point-tooltip" style={style}>{children}</div>;
}
