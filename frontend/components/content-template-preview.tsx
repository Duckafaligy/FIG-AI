...
export function ContentTemplatePreview({ title, category, topic, template, revision, id }: Props) {
  ...
  return <article className={`detail-cms-frame detail-cms-frame--${kind}`}>
    <div className="detail-cms-nav"><strong>LaunchVault</strong><span>Learn AI</span><span>Guides</span><span>Workflow library</span><a href={`#${sectionId(0)}`}>Start learning</a></div>
    <div className="detail-cms-hero">
      <div><span>{kind === "tutorial" ? "Hands-on tutorial" : category}</span><h4>{title}</h4><p>{revision % 2 === 0 ? `Put ${topic} into practice with a small, repeatable workflow. Follow the example, check the result, and make it your own.` : `Go from a blank page to a useful first result. This guide breaks ${topic} into clear decisions, a worked example, and a review checklist.`}</p><div><small>By LaunchVault team</small><small>·</small><small>May 25, 2025</small><small>·</small><small>4 min read</small></div></div>
      <div className="detail-hero-art" role="img" aria-label="LaunchVault learning guide cover"><span>LV</span><i /><i /><i /></div>
    </div>
    ...
    <div className="detail-cms-content">
      <div className="detail-cms-article">
        ...
        <div className="detail-cms-callout"><Sparkles size={16} /><span>LaunchVault tip</span><p>Use a short, non-sensitive example for your first test. Check the process before you use it for important work.</p></div>
    ...
    <div className="detail-cms-footer"><strong>LaunchVault</strong><span>Practical AI skills. One useful step at a time.</span></div>
  </article>;
}
