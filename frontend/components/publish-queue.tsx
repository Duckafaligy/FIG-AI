...
export function PublishQueue() {
  ...
  const propose = async () => {
    setProposing(true);
    const result = await actions.proposeChanges();
    setProposing(false);
    if (result.ok) load();
  };
    ...
    <div className="publish-queue">
      ...
      {data.sites > 0 && data.connected === 0 && (
        <div className="pq-note">
          <p>No CMS is connected on any of your sites yet. Proposed changes can still be reviewed and approved
            here, but Publish stays disabled until you connect one under Settings → Integrations.</p>
        </div>
      )}
      ...
      {data.sites > 0 && rows.length === 0 && (
        <div className="pq-empty">
          <p>Nothing proposed yet. Click <strong>Propose changes</strong> to check your sites' latest scans for
            fixes FIG can apply mechanically — titles, heading structure, and (where a platform supports it)
            meta descriptions.</p>
        </div>
      )}
