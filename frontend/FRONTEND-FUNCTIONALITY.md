# FIG frontend functionality reference

**Snapshot:** September 15, 2026
**Audience:** product, design, frontend, and backend handoff
**Demo workspace:** one illustrative project — **LaunchVault.ca**
...
| **Read-only preview** | Shows illustrative LaunchVault.ca information only. |
| **Planned integration** | Opens an explanatory dialog or status message. It never sends data to an external service. |

- All dashboard charts, counts, content titles, notifications, scores, users, and status labels are illustrative data for LaunchVault.ca.
...
| `/projects` | Project chooser | The one LaunchVault.ca project, trend, calendar, and workspace summary. |
...
This is the only project selector in the demo. It is scoped to one LaunchVault.ca card so the visual layout stays believable before customer records exist.

| Control / section | Current behavior |
| --- | --- |
| FIG / LaunchVault brand | Links back to `/projects`. |
...
| **New project** / **Create new project** | Opens a native dialog explaining that this preview has one project. **Open LaunchVault** routes to `/app`; close controls dismiss the dialog. No project is created. |
...
| LaunchVault.ca card | Navigates to `/app`. |
...
| Control | Current behavior |
| --- | --- |
| Sidebar LaunchVault logo | Returns to `/projects`. |
...
| Workspace/date controls | Label the fixed LaunchVault.ca preview and illustrative date range. They are not switches. |
...
The frontend is a standalone Next.js app inside the repository’s `frontend` directory. The exact Vercel setup, environment-variable boundary, and production verification command are in [VERCEL-DEPLOYMENT.md](./VERCEL-DEPLOYMENT.md).
