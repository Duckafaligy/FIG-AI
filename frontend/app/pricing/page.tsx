...
const plans = [
  { name: "Starter", price: "$29", description: "For individuals and small teams getting started.", cta: "Get started", features: ["5 projects", "1 CMS connection", "AI-assisted creation", "SEO review", "Basic analytics", "Up to 3 seats"] },
  { name: "Pro", price: "$79", description: "For growing teams with a steady content rhythm.", cta: "Start free", popular: true, features: ["25 projects", "5 CMS connections", "Advanced generation", "SEO + GEO workflows", "Full analytics", "Up to 10 seats"] },
  { name: "Scale", price: "$199", description: "For larger teams with complex content operations.", cta: "Start free", features: ["100 projects", "10 CMS connections", "Advanced generation", "SEO + GEO workflows", "Automation rules", "Up to 25 seats"] },
      ...
      <main>
        <section className="pricing-hero section-glow">
          <div className="page-shell pricing-hero-grid">
            <div><span className="section-kicker">Pricing</span><h1>Simple, transparent pricing for teams that create content that <span>gets found</span></h1><p>Choose the plan that fits your team. Every plan begins with the same calm, connected FIG workflow.</p><div className="billing-toggle"><button className="active">Monthly</button><button>Yearly</button><span>Save up to 20%</span></div><div className="pricing-trust"><span><Check size={15} />No credit card required</span><span><Check size={15} />Change plans anytime</span></div></div>
        ...
        <section className="section page-shell">
          ...
          <div className="plan-grid">{plans.map((plan) => <article className={plan.popular ? "popular" : ""} key={plan.name}>{plan.popular && <span className="popular-tag">Most popular</span>}<span className="plan-icon">{plan.name === "Starter" ? <Sparkles /> : plan.name === "Pro" ? <BarChart3 /> : plan.name === "Scale" ? <Layers3 /> : <ShieldCheck />}</span><h3>{plan.name}</h3><p>{plan.description}</p><strong className="plan-price">{plan.price !== "Custom" && plan.price}<span>{plan.price === "Custom" ? "Custom pricing" : "/month"}</span></strong><ul>{plan.features.map((feature) => <li key={feature}><Check size={16} />{feature}</li>)}</ul><Link className={plan.popular ? "button" : "secondary-button"} href="/signup">{plan.cta}</Link></article>)}</div>
        </section>
